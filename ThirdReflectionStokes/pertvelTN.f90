...
if(flagb <XN+1.0 ) then
!print *, "w0"
call trapzN(w2IntegrandBackflow, xflagb, sx(XN+1), numtrapz, tempbackflow)

endif
if (flagu /= 0.0) then
!print *, "w1"
call trapzN(w2IntegrandPartialSphere, real(0.0, kind=8), xflagl, numtrapz, tempbelowsphere)
!print *, "w2"
call trapzN(w2IntegrandZetaSphere, max(-R, sy(1)), R, numtrapz, tempsidesphere)
!print *, "w3"
call trapzN(w2IntegrandZetaVert, R, yend, numtrapz, tempabovesphere)
!print *, "w3.5"
elseif (flagl /= 0.0) then
!print *, "w4"
call trapzN(w2IntegrandPartialSphere, real(0.0, kind=8), xflagl, numtrapz, tempbelowsphere)
!print *, "w5"
call trapzN(w2IntegrandZetaSphere, max(-R, sy(1)), yend, numtrapz, tempsidesphere)
!print *, "w5.5"
else
!print *, "w6"

!print *, "below sphere limits", real(0.0, kind=8), xflagb
call trapzN(w2IntegrandBelowSphere, max(sx(1), real(0.0, kind=8)), xflagb, numtrapz, tempbelowsphere)
...
!print *, "w1"
myrho   = rho
call trapzN(w2IntegrandZeta, yend, zcoord, numtrapz, w2IntegrandBackflow)
end
...
!print *, "w2"
xlower  = sqrt(R**2 - zeta**2)
call trapzN(w2IntegrandRho, xlower, xupper, numtrapz, w2IntegrandZetaSphere)
end
...
call interpbridge(min(flagl+3, XN+1), sx(1:min(flagl+3, XN+1)), sy(1:min(flagl+3, XN+1)), rho, zcoord)
!print *, "w3"
call trapzN(w2IntegrandZeta, zcoord, -R, numtrapz, w2IntegrandPartialSphere)
end
...
!print *, "w4"
!print *, yend
call trapzN(w2IntegrandRho, real(0.0, kind=8), xupper, numtrapz,w2IntegrandZetaVert)
end
...
call trapzN(w2IntegrandZeta, zcoord, yend, numtrapz, w2IntegrandBelowSphere)
end
...
call trapzN(LogTermThetaR,  real(0.0,kind=8),real(2.0*pi,kind=8), 10, ILogR)

else
call trapzN(LogTermThetaR,  real(0.0,kind=8),real(2.0*pi,kind=8), 50, ILogR)
endif
...
if (abs (myzeta*px + py * myrho ) < 0.1 .and. abs (myzeta*px + py * myrho ) > 0.0) then


call trapzN(LogTermThetaZ,real(0.0,kind=8),real(2.0*pi,kind=8),10, ILogZ)


!call simp2(LogTermThetaZ, real(0.0,kind=8), real(2.0*pi,kind=8),integthres, ILogZ)
elseif (abs (myzeta*px + py * myrho ) > 0.1 )then

call trapzN(LogTermThetaZ, real(0.0,kind=8),real(2.0*pi,kind=8),50, ILogZ)
